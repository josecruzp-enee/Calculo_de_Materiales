# -*- coding: utf-8 -*-
"""
entradas_pdf.py

Entrada desde PDF.
Aquí solo se extrae información cruda (pendiente definir tu método exacto).
"""

from __future__ import annotations

from typing import Dict, Any, Tuple
import pandas as pd


def cargar_desde_pdf(datos_fuente: Dict[str, Any]) -> Tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Espera en datos_fuente:
        - "archivo_pdf": ruta o bytes
        - "datos_proyecto": dict (opcional)

    Retorna:
        datos_proyecto, df_estructuras, df_cables, df_materiales_extra
    """
    datos_proyecto = dict(datos_fuente.get("datos_proyecto") or {})

    # Pendiente: extracción desde PDF (cuando definamos tu estrategia)
    df_estructuras = pd.DataFrame()
    df_cables = pd.DataFrame()
    df_materiales_extra = pd.DataFrame()

    return datos_proyecto, df_estructuras, df_cables, df_materiales_extra
